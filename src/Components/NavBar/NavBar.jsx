...
import { Logo, NavBarContainer, LinkContainerDesktop } from "./NavBar.style";
    ...
    >
      ...
      <LinkContainerDesktop>
        {routes.map((el, index) => (
         ...
         {user?<><a onClick = {()=>navigate(`/profile`)}>Profile</a> <a onClick={handleLogout}>Logout</a></>: <a onClick = {()=>navigate(`/login`)}>Login</a>}
      </LinkContainerDesktop>
    </NavBarContainer>
  );
}
